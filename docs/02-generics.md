...
| Constraint | Описание | Примечание |
|---|---|---|
| `where T : struct` | T -- value type (исключая Nullable) | Исключает Nullable<T>. Должен быть первым.
| `where T : class` | T -- reference type | Включает классы, интерфейсы, делегаты.
| `where T : class?` | T -- nullable reference type | Позволяет передавать null без предупреждений компилятора.
| `where T : notnull` | T -- non-nullable (value или reference) | Работает и для value, и для reference типов (C# 8.0+).
| `where T : unmanaged` | T -- unmanaged type (int, byte, struct без ссылок) | Типы, которые не содержат ссылок (blittable). Полезно для работы с памятью/указателями.
| `where T : new()` | T имеет публичный конструктор без параметров | Если указано, T нельзя инициализировать через abstract. Должно быть последним в списке.
| `where T : BaseClass` | T наследуется от BaseClass | Не может быть sealed классом, Static, System.Array и т.д.
| `where T : BaseClass?` | Наследование от класса (nullable) | Допускает передачу null в качестве аргумента типа.
| `where T : IInterface` | T реализует IInterface | Можно указывать несколько интерфейсов через запятую.
| `where T : U` | T наследуется от другого параметра U | Называется «Naked type constraint». Удобно для сопоставления типов в методах.
| `where T : Enum` | T -- перечисление  | (C# 7.3+)
| `where T : Delegate` | T -- делегат (C# 7.3+) | (C# 7.3+)
| `where T : MulticastDelegate` | T -- Ограничение на многоадресные делегаты. | (C# 7.3+)

### 3.1. Основные ограничения (Primary Constraints)
Они определяют фундаментальную природу типа. В списке ограничений для одного параметра может быть только одно такое ограничение, и оно всегда идет первым.

| Constraint | Описание | Примечание
|---|---|---|
| `where T : struct` | Тип значения (Value type). | Исключает Nullable<T>. Должен быть первым.
| `where T : class` | Ссылочный тип (Reference type). | "Включает классы, интерфейсы, делегаты."
| `where T : class?` | Ссылочный тип (nullable). | Позволяет передавать null без предупреждений компилятора.
| `where T : notnull` | Не допускает null. | "Работает и для value, и для reference типов (C# 8.0+)."
| `where T : unmanaged | Неуправляемый тип. | "Типы, которые не содержат ссылок (blittable). Полезно для работы с памятью/указателями."

### 3.2. Ограничения наследования (Secondary Constraints)
Указывают, от чего должен наследоваться T или какие интерфейсы реализовывать.

| Constraint | Описание | Примечание
|---|---|---|
| `where T : BaseClass` | Наследование от класса. | "Не может быть sealed классом, Static, System.Array и т.д."
| `where T : BaseClass?` | Наследование от класса (nullable). | Допускает передачу null в качестве аргумента типа.
| `where T : IInterface` | Реализация интерфейса. | Можно указывать несколько интерфейсов через запятую.
| `where T : U` | Наследование от другого параметра. | Называется «Naked type constraint». Удобно для сопоставления типов в методах.

### 3.3. Специальные системные ограничения
Ограничения на специфические типы платформы .NET.

| Constraint | Описание | Примечание
|---|---|---|
| Constraint | Описание | С# версии
| `where T : Enum` | Ограничение на перечисления. | C# 7.3
| `where T : Delegate` | Ограничение на делегаты. | C# 7.3
| `where T : MulticastDelegate` | Ограничение на многоадресные делегаты. | C# 7.3

### 3.4. Ограничение конструктора

| Constraint | Описание | Примечание
|---|---|---|
| `where T : new()` | Наличие публичного конструктора без параметров. | "Если указано, T нельзя инициализировать через abstract. Должно быть последним в списке."

### 3.5. Анти-ограничение (C# 13+)
Это особенное расширение возможностей дженериков, которое появилось недавно.

| Constraint | Описание 
|---|---|
| `allows ref struct` | "Позволяет использовать `ref struct` (например, `Span<T>`) в качестве параметра типа. По умолчанию дженерики их запрещают."

### Важные правила комбинирования
При написании кода важно соблюдать строгий порядок:

1. **Основное ограничение** (`struct, class, notnull, unmanaged` или базовый класс).
2. **Интерфейсы.**
3. **Ограничение конструктора** (`new()`).
  
### Пример сложного ограничения:
```csharp
public class Repository<T> 
    where T : BaseEntity, IAggregateRoot, new()
{
    // T должен быть наследником BaseEntity, реализовать IAggregateRoot и иметь конструктор
}
```

> [!TIP]
> Совет: Если вы работаете с высокопроизводительным кодом, обратите внимание на where T : unmanaged. Это позволяет использовать T* (указатели) внутри generic-методов, что критично для библиотек обработки данных.
...
> [!TIPS] **Совет**: Если вы работаете с высокопроизводительным кодом, обратите внимание на where T : unmanaged. Это позволяет использовать T* (указатели) внутри generic-методов, что критично для библиотек обработки данных.
